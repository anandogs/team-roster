...
Alpine.store("filters", {
    ...
    // Initialization
    async init() {
        const [filterResponse, periodResponse, customersResponse] = await Promise.all([
        ...
        this.customers = [...new Set(customersData.map(c => c.FinalCustomer))];
        this.selectedCustomers = [...this.customers];
        this.selectedLocations = [...this.availableLocations];
        this.selectedBillableStatus = [...this.availableBillableStatus.map(s => s.value)];
        this.selectedBusinessUnits = [...this.businessUnits];

        if (periodData.QTR) {
            document.querySelector('.bg-neutral-800 .text-white').textContent = periodData.QTR;
        }
    ...
    populateBusinessUnitOptions() {
        ...
        if (buOptions) {
            buOptions.innerHTML = this.businessUnits.map(bu => `
                <button
    ...
    populateMonthOptions() {
        ['M1', 'M2', 'M3'].forEach(monthKey => {
            const button = document.querySelector(`button[onclick="selectMonth('${monthKey}')"]`);
            if (button && this.periodData[monthKey]) {
                button.onclick = () => this.selectMonth(monthKey, this.periodData[monthKey]);
                button.innerHTML = `
                    <svg class="mr-2 h-4 w-4 opacity-0" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path d="M20 6L9 17l-5-5" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
    ...
    toggleBusinessUnit(bu, isChecked) {
        if (isChecked) {
            ...
            this.selectedBusinessUnits = this.selectedBusinessUnits.filter(b => b !== bu);
        }
    ...
    updateSelectAllBusinessUnitState() {
        const selectAllCheckbox = document.getElementById('select-all-business-units');
        if (!selectAllCheckbox) return;
        
        const allSelected = this.selectedBusinessUnits.length === this.businessUnits.length;
    ...
    updateBusinessUnitDisplay() {
        ...
        } else if (this.selectedBusinessUnits.length === this.businessUnits.length) {
    ...
    updateCustomerOptions() {
        ...
        if (customerOptions) {
            customerOptions.innerHTML = this.customers.map(customer => `
                <label class="flex items-center px-2 py-1.5 text-sm text-white hover:bg-neutral-700 rounded-sm cursor-pointer">
                    <input
                        type="checkbox"
                        value="${customer}"
                        ...
                        onchange="toggleCustomer('${customer}', this.checked)"
                        ${this.selectedCustomers.includes(customer) ? 'checked' : ''}
                    >
    ...
    updateSelectAllState() {
        ...
        const allSelected = this.selectedCustomers.length === this.customers.length;
    ...
    updateLocationOptions() {
        ...
        if (locationOptions) {
            locationOptions.innerHTML = this.availableLocations.map(location => `
                <label class="flex items-center px-2 py-1.5 text-sm text-white hover:bg-neutral-700 rounded-sm cursor-pointer">
                    <input
                        type="checkbox"
                        value="${location}"
                        ...
                        ${this.selectedLocations.includes(location) ? 'checked' : ''}
                    >
    ...
    updateSelectAllLocationState() {
        ...
        const allSelected = this.selectedLocations.length === this.availableLocations.length;
    ...
    updateLocationDisplay() {
        ...
        if (this.selectedLocations.length === 0) {
            ...
            if (clearButton) clearButton.classList.remove('hidden');
        } else if (this.selectedLocations.length === this.availableLocations.length) {
    ...
    updateBillableOptions() {
        ...
        if (billableOptions) {
            billableOptions.innerHTML = this.availableBillableStatus.map(status => `
                <label class="flex items-center px-2 py-1.5 text-sm text-white hover:bg-neutral-700 rounded-sm cursor-pointer">
                    <input
                        type="checkbox"
                        value="${status.value}"
                        ...
                        onchange="toggleBillableStatus('${status.value}', this.checked)"
                        ${this.selectedBillableStatus.includes(status.value) ? 'checked' : ''}
                    >
    ...
    toggleBillableStatus(statusValue, isChecked) {
        if (isChecked) {
            ...
            this.selectedBillableStatus = this.selectedBillableStatus.filter(s => s !== statusValue);
        }
    ...
    updateSelectAllBillableState() {
        ...
        const allSelected = this.selectedBillableStatus.length === this.availableBillableStatus.length;
    ...
    updateBillableDisplay() {
        ...
        if (this.selectedBillableStatus.length === 0) {
            ...
            if (clearButton) clearButton.classList.remove('hidden');
        } else if (this.selectedBillableStatus.length === this.availableBillableStatus.length) {
            ...
            const selectedStatus = this.availableBillableStatus.find(s => s.value === this.selectedBillableStatus[0]);
            displayElement.textContent = selectedStatus ? selectedStatus.label : this.selectedBillableStatus[0];
            if (clearButton) clearButton.classList.remove('hidden');
        } else {
